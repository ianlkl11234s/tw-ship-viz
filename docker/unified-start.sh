...
0 7 * * * root . /etc/environment; cd /app && python3 scripts/import_to_db.py --incremental >> /var/log/cron.log 2>&1 && python3 scripts/generate_arrow.py --days 7 >> /var/log/cron.log 2>&1
CRON
...
if [ ! -f frontend/public/data/trajectory.arrow ]; then
  ...
  if [ -f data/ship_data.db ]; then
    python3 scripts/generate_arrow.py --days 7 2>&1 || echo "[ship-gis] Arrow 產出失敗（可能資料庫為空）"
  else
